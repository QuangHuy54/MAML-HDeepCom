...
import torch
torch.manual_seed(1)
def _train(testing_project,training_projects,is_transfer,num_fold,validating_project,vocab_file_path=None, model_file_path=None,model_state_dict=None,num_of_data=-1,seed=1,adam=True):
    ...
    if not is_transfer:
        ...
        train_instance = train.Train(vocab_file_path=vocab_file_path,code_path=f'../dataset_v2/original/{testing_project}/fold_{num_fold}_train.code'
                                    ...
                                    ,num_of_data=num_of_data,save_file=False,seed=seed,adam=adam,is_full_baseline=True,training_projects=training_projects)        
...
if __name__ == '__main__':
    ...
    path = args.path
    validating_project=project2validate[testing_project]
        ...
        if args.specific==None:
            for file in dir_list:
                ...
                for num_fold in range(5):
                    ...
                    for i in range(num_test):
                        best_model_dict2=_train(testing_project,training_projects=project2sources[testing_project],is_transfer=True,vocab_file_path=(config.code_vocab_path, config.ast_vocab_path, config.nl_vocab_path),model_file_path=os.path.join(path,file),num_of_data=num_data,seed=i,adam=args.adam,num_fold=num_fold,validating_project=validating_project)
            ...
            for num_fold in range(5):
                ...
                for i in range(num_test):
                    ...
                    if num_data!=0:
                        best_model_dict2=_train(testing_project,training_projects=project2sources[testing_project],is_transfer=True,vocab_file_path=(config.code_vocab_path, config.ast_vocab_path, config.nl_vocab_path),model_file_path=os.path.join(path,args.specific),num_of_data=num_data,seed=i,adam=args.adam,num_fold=num_fold,validating_project=validating_project)
                    result=_test(best_model_dict2,testing_project,num_fold=num_fold)      
                    if res_dict==None:
                        res_dict=result
                    else:
