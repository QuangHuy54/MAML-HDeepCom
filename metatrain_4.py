...
class MetaTrain(object):
    ...
    def __init__(self, training_projects, validating_project,tesing_project, vocab_file_path=None, model_file_path=None,lr=0.05,save_path=None,num_of_data=-1):
        """
        ...
        self.meta_datasets = {}
        self.similarity_semantic={}
        original_code_semantic=torch.load(os.path.join(dataset_dir,f'{tesing_project}/all_code_semantic.pt'), map_location='cpu')
        original_code_semantic=torch.max(original_code_semantic, 0).values
        #print(projects)
        for project in training_projects+[validating_project]:
            target_semantic=torch.load(os.path.join(dataset_dir,f'{project}/all_code_semantic.pt'), map_location='cpu')
            target_semantic=torch.max(target_semantic, 0).values
            self.similarity_semantic[project]=torch.cosine_similarity(original_code_semantic,target_semantic,-1).item()
